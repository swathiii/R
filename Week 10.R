...
df <- data4 %>% 
  filter(if_all(everything(), ~! (.x %in% c(-1, 98, 99, 8))))
...
data4$Spend1[ data4$Spend1 == -1 ] <- NA
data4$Spend1[ data4$Spend1 == 98 ] <- NA

data4 <- na.omit(data4)

data_viz <- data4[,c(12, 13, 16)]
data_viz <- data4[,16]
head(data_viz)
View(data_viz)
#plotting the heatmap using geom_tile
ggplot(data_viz, aes(SMNews, PartyId2)) +
  geom_tile(aes(fill = Spend1), colour = "black") +
  scale_fill_gradient(low = 'steelblue', high = 'pink')

#-----heatmap for SMNews and Spend1
data_viz2 <- data4[,c(12, 16)]
View(data_viz2)

ggplot(data_viz2, aes(SMNews, Spend1)) +
  geom_tile(aes(fill = SMNews), colour = 'black') +
  scale_fill_gradient(low = 'steelblue', high = 'pink')
...
# Visualise the correlation matrix for the following variables: 
#TaxSpend, SocialTrust, 'MuchPov', Poverty1, Poverty2, and PartyId2. 
#Do you notice any interesting patterns

cor_plot <- cor(data4[ ,c('TaxSpend', 'SocTrust', 'MuchPov',
                          'Poverty1', 'Poverty2', 'PartyId2')])
corrplot(cor_plot)
